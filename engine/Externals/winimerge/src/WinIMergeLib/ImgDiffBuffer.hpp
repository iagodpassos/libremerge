...
public:
	...
	void reverse()
	{
#ifndef LM_IMAGE_SCANLINES_TOPDOWN
...
#ifdef LM_IMAGE_SCANLINES_TOPDOWN
	// LibreMerge: the Qt image backend stores scanlines top-down and
	// contiguous, so row 0 is already the first record in memory
	const char* data() const { return reinterpret_cast<const char *>(m_img.scanLine(0)); }
#else
